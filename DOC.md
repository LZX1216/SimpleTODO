...
- 模块职责说明。  
  ...
  - **frontend/src/App.vue**: Vue 3 主应用组件，包含主要业务逻辑和 UI 渲染，包括导入/导出功能。
  - **frontend/src/components/TaskCard.vue**: 任务卡片组件，封装任务显示和编辑逻辑，提高代码复用性。
  - **frontend/src/utils/**: 工具函数目录，包含常量配置、日期处理、数据验证等工具函数。
...
- 任务描述展开/收起功能
  ...
  - 描述内容支持换行显示，保留原始格式。
- 任务列表展开/收起功能
  - 待处理任务列表默认展开，已完成任务列表默认收起，可以通过点击标题展开/收起。
  - 两个列表的展开/收起状态相互独立。
- 快速回到顶部功能
  ...
  - 按钮带有淡入淡出动画效果。
- 响应式布局优化
  - 宽屏幕（≥1024px）：左右分栏布局，左侧固定宽度350px，右侧自适应。
  - 窄屏幕（<1024px）：
    - 单栏布局，所有内容垂直排列
...
- AI 输出如何修改：  
  ...
  - AI 输出的代码在数据处理和验证上存在潜在遗漏，结合自己的业务理解手动补充校验逻辑，而不是直接照搬生成结果。
  - 在导出和导入功能中，AI 只给出基础的实现方式。参考实际使用场景，补充了导出文件命名、格式校验、合并数据等细节，提升用户体验。
  - AI 生成的部分 API 示例接口返回字段不完全贴合前端需要，能判断并根据具体需求手动调整后端接口返回的数据结构，从而提高前后端协作效率。
  - 修改了 AI 生成的前端内容，使其符合使用习惯。
...
- 如果有更多时间，你会如何改进？
  ...
  - **前端组件化重构**：已部分完成，提取了 `TaskCard` 组件和工具函数（`utils/`），减少了约270行重复代码。未来可进一步拆分为 TaskForm、SearchFilter、StatsPanel 等组件。
